...
//Использовал Фабричный метод, для создания Train Plane Car с обязательным переопределением общих методов
//Использовал класс - интерфейс Мост, для обязательного связующего метода best(xxx);
...
class FactoryMethodCar {
	int speed;
	int volume;
	double price;
	int distance;
public:
	virtual ~FactoryMethodCar() {};
	virtual double sumCost(int, int) = 0;
	virtual double sumTime(int) = 0;
	virtual int getDistance() = 0;
};

class Train : FactoryMethodCar
{
	int trainSpeed;
	int trainVolume;
	double trainPrice;
...
class Bridge {
public:
	virtual Track best(std::string, std::string, Type, int) {};
};
